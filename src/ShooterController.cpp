...
double getRPM(Counter* counter) { return 60.f / (16.f * counter->GetPeriod()); }
...
float ShooterController::DetermineMotorSpeedFromRPM(float fRPM) {
    ...
    if (fRPM == 0.0F) {
        ...
        for (index = 1; index < m_motorSpeedTableLength; index++) {
            ...
            float previousRotations =
                m_motorSpeedTable[indexPrevious].rotationsPerMinute;
            float currentRotations =
                m_motorSpeedTable[index].rotationsPerMinute;

            if (fRPM >= previousRotations && fRPM <= currentRotations) {
                // Interpolate between previous and current table entries.
                float p = (fRPM - previousRotations) /
                          (currentRotations - previousRotations);
                float previousMotorValue =
                    m_motorSpeedTable[indexPrevious].motorValue;
                result = previousMotorValue +
                         p * (m_motorSpeedTable[index].motorValue -
                              previousMotorValue);
                break;
            }
        }

        // Should always find an entry, but just in case, use largest possible
        // value.
...
float ShooterController::DetermineRPMFromTargetDistance(float distanceFt) {
    ...
    if (distanceFt > 0.0F) {
        ...
        for (unsigned int index = 1; index < m_distanceRPMTableLength;
             index++) {
            // Interpolate between previous and current table entries.
            unsigned int indexPrevious = index - 1;
            float previousDistance =
                m_distanceRPMTable[indexPrevious].targetDistanceFt;
            float currentDistance = m_distanceRPMTable[index].targetDistanceFt;
            if (distanceFt >= previousDistance &&
                distanceFt <= currentDistance) {
                float p = (distanceFt - previousDistance) /
                          (currentDistance - previousDistance);
                float previousRPMValue =
                    m_distanceRPMTable[indexPrevious].rotationsPerMinute;
                fResultRPM = previousRPMValue +
                             p * (m_distanceRPMTable[index].rotationsPerMinute -
                                  previousRPMValue);
                break;
            }
        }
...
void ShooterController::Initialize(Counter* pEncoder, Victor* pShooterMotorLeft,
...
void ShooterController::InitializeDistanceRPMTable() {
    ...
    m_distanceRPMTable =
        new TargetDistanceRPMTableEntry[m_distanceRPMTableLength];
...
void ShooterController::InitializeMotorSpeedTable() {
    ...
    // By definition, the last table entry is the maximum possible shooter
    // rotational speed.
...
void ShooterController::ResetCalibration() {
    ...
    if (m_bGotGoodEncoderValue) {
        ...
        // Since we have no encoder value to go on, just set the output motors
        // to the target value.
...
void ShooterController::SetShooterRPM(float rpm) {
    if (rpm < ZERO_RPM_BAND) rpm = 0.0F;

    // If target RPM is (significantly) different from current target, or
    // zero...
...
void ShooterController::Update() {
    ...
    if (m_fTargetRPM < ZERO_RPM_BAND) {
        ...
        // ADJUSTMENT = (ERROR(P) * GAIN(P)) + (ERROR(D) * GAIN(D)) +
        // ((ERROR(I) * GAIN(I))

        float adjustRPMs = fErrorRPM.RPM * GAIN_KP;

        // Add integral control error, which is sum of all errors since output
        // target was set.
        m_fAccumulatedError +=
            fErrorRPM.RPM * (fErrorRPM.time - m_fPreviousError.time);
        adjustRPMs += m_fAccumulatedError * GAIN_KI;

        // Add derivative error, which is difference between this error and the
        // previous cycle's error.
        adjustRPMs += (fErrorRPM.RPM - m_fPreviousError.RPM) /
                      (fErrorRPM.time - m_fPreviousError.time) * GAIN_KD;

        // 'adjustRPMs' is now the total error adjustment.
        // After error adjustment, this is what we want to set the motor speed
        // to
        ...
        m_bIsReady = (fErrorRPM.RPM > -RPM_RANGE_BAND) &&
                     (fErrorRPM.RPM < RPM_RANGE_BAND);
    }
}
