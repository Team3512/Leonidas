...
int sockets_readh(struct list_t* list, struct list_elem_t* elem) {
    struct socketconn_t* conn = elem->data;
    int error;
    ...
    error = recv(conn->fd, conn->readbuf,
...
int sockets_readdoneh(uint8_t* inbuf, size_t bufsize, struct list_t* list,
                      struct list_elem_t* elem) {
    struct socketconn_t* conn = elem->data;
    char* buf;
