...
PhotoEncoder::PhotoEncoder(uint32_t channel, uint32_t teeth, float gearRatio)
    : m_counter(channel),
      m_rpmAverager(2),
...
void PhotoEncoder::start() { m_counter.Start(); }

void PhotoEncoder::stop() { m_counter.Stop(); }

void PhotoEncoder::setTeethPerRevolution(uint32_t teeth) { m_teeth = teeth; }

void PhotoEncoder::setGearRatio(float ratio) { m_gearRatio = ratio; }

void PhotoEncoder::setAverageSize(uint32_t size) {
    m_rpmAverager.setSize(size);
}

float PhotoEncoder::getRPM() { return m_rpmAverager.getAverage(); }
...
void PhotoEncoder::setSampleRate(uint32_t sampleRate) {
