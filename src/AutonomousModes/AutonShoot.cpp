    ...
    while (IsEnabled() && IsAutonomous()) {
        ...
        if (aiming) {
            ...
            if (std::fabs(turretKinect.getPixelOffset()) >
                TurretKinect::pxlDeadband) {
                /* Set(x) = x / 320
                 * x is always in range: -320 <= x <= 320,
                 * therefore Set( x / 320.f ) always gets a value between -1 and
                 * 1
        ...
        if (autoTime.Get() > 6.f) {  // gives shooter six seconds to spin up
                                     // before starting ball conveyor
