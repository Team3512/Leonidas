 ...
 * The shooter can be controlled by setting either the rotational speed (SetShooterRPM) or
 * the distance to the target (SetTargetDistance).
 * Call one of these methods when a change in rotation speed or target distance is known.
 *
 * After setting or changing the target, call the 'Update' method during each processing cycle.
 * This class will set the shooter output motor speeds as necessary.
 *
 * Call the 'IsReady' method to determine whether the shooter has stabilized at the desired
 * rotational speed.
 *
 * In order to stop the shooter from rotating, set the rotational speed or distance to zero.
 */
...
public:
	...
	//! Set the target rotational speed of the shooter, in rotations per minute (RPM).
...
private:
	...
	TargetDistanceRPMTableEntry*	m_distanceRPMTable;		// an array of 'TargetDistanceRPMTableEntry'
...
#endif	// _SHOOTER_CONTROLLER_INCLUDED
